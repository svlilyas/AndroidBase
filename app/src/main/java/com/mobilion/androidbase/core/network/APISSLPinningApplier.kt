...
class APISSLPinningApplier : SSLPinningApplier {
    ...
    override fun apply(clientBuilder: OkHttpClient.Builder, assetManager: AssetManager, certificatePath: String) {
        apply(clientBuilder, assetManager, listOf(certificatePath))
    }

    override fun apply(clientBuilder: OkHttpClient.Builder, assetManager: AssetManager, certificatePaths: List<String>) {
    ...
    override fun apply(clientBuilder: OkHttpClient.Builder, apiURL: String, publicKeys: List<String>) {
        if (publicKeys.isEmpty() || apiURL == null) {
            return
        }
    ...
    fun getCertificatePaths(): List<String> {
        if (BuildConfig.DEBUG) return emptyList()
        if (TextUtils.isEmpty(BuildConfig.SERVICE_CERTIFICATE_PATH)) return emptyList()
        return BuildConfig.SERVICE_CERTIFICATE_PATH.split(",")
    }
}